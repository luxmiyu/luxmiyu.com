...
<Container fill>
  ...
  <Text>
    <p class="center">
      <Input.Select bind:value={rhythmIn} width="72px">{@render options()}</Input.Select>
      at
      <Input.Number bind:value={bpmIn} min={1} max={99999} step={1} width="72px" />
      BPM is equivalent to
      <Input.Select bind:value={rhythmOut} width="72px">{@render options()}</Input.Select>
      at
      <Output value={bpmOut.toFixed(1)} width="72px" />
      BPM.
    </p>
